...
from OGshipindanmu import Ui_MainWindow
...
#
    def initUI(self):
        #子窗口名字修改
        self.setWindowTitle("欢迎使用bilibili视频弹幕爬取程序")
        #子窗口最大化
        self.showMaximized()
        self.textBrowser_3.append("欢迎使用bilibili视频弹幕爬取程序，请设置爬虫参数\r")

# #       #lineEdit验证器 只允许数字
#         self.lineEdit_1.setValidator(QIntValidator(1, 9999999))
...
#         #定义接口数据
        ...
        #更改swichbtn状态
        self.select_btn = 1
        self.select_btn_translate = "单一av号爬取"
        ...
        if self.pushButton_1.text() == "取消":
            ...
            if self.interface_data[1] == 1:
                if self.interface_data[0] == "":
                    self.textBrowser_3.append("av号不能为空\r")
...
#
    ...
    def showtable(self, a):
        ...
        self.sheet.write(self.book_row, 1, a["av"])
        self.sheet.write(self.book_row, 2, a["str"])
...
#
    ...
    def get_name(self, av):
        a = self.get_text("https://www.bilibili.com/video/av%s" % av)
        b = re.search('"cid":(.*?),', a, re.S)
        if b == None:
            self.error_thread.emit("av号：%s 无视频，请检查\r" % av)
        else:
            b = re.findall('"cid":(.*?),', b.group(), re.S)[0]
            c = "https://api.bilibili.com/x/v1/dm/list.so?oid=%s" % b
            d = self.get_text(c)
        ...
        if self.identity[1] == 1:
            ...
            for i in range(len(x)):
                try:
                    ...
                    self.error_thread.emit("第一列不全为整数的数字，请第%s行\r" % (i+1))

                try:
                    self.error_thread.emit("正在爬取excel第%s行,av号:%s\r" % (i+1, ii))
                    self.get_name(ii)
                except:
                    self.error_thread.emit("bilibili被禁止访问，请等待五分钟后再开始\r")
                    self.working = False
